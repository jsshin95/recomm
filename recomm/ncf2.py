...
print('min_u max_u min_i max_i : ',data['user_id'].min(),data['user_id'].max(),data['item_id'].min(),data['item_id'].max())

# Adjust user_id and item_id to start from 0
data['user_id'] -= 1
data['item_id'] -= 1
...
print("Model saved to 'ncf_model.h5'")
...
print("Model loaded from 'ncf_model.h5'")

# Make predictions
user_id = 22  # Example user ID
item_id = 377  # Example item ID

# Convert user_id and item_id to 0-based indexing
user_id -= 1
item_id -= 1
...
user_id_array = np.array([195,185,21,243,165,297,114])
item_id_array = np.array([241,301,376,50,345,473,264])

predicted_rating = model.predict([user_id_array, item_id_array])
#print(f'Predicted rating for user {user_id + 1} and item {item_id + 1}: {predicted_rating[0][0]}')
print(predicted_rating) # 3 3 1 2 1 4 2